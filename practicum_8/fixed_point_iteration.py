...
def relaxation_method(A: NDArrayFloat, b: NDArrayFloat, omega: float, n_iters: int) -> NDArrayFloat:
    D = np.diag(np.diag(A))
    L = -np.tril(A, k=-1)
    U = -np.triu(A, k=1)
    D_omega_L_inv = np.linalg.inv(D - omega*L)
    T = D_omega_L_inv @ ((1-omega)*D + omega*U)
    c = omega*D_omega_L_inv @ b
    return fixed_point_iteration(T, c, n_iters)
