...
def conjugate_gradient_method(
    ...
    r = b - A @ x  # Невязка
    v = r  # Направление
    solution_history = np.zeros((n_iters, n)) if n_iters is not None else None

    for k in range(n_iters):
        t = np.dot(r, r) / np.dot(v, A @ v)
        x = x + t * v
        r_new = r - t * A @ v
        s = np.dot(r_new, r_new) / np.dot(r, r)
        v = r_new + s * v
        r = r_new

        if solution_history is not None:
            solution_history[k] = x
...
def preconditioned_conjugate_gradient_method(
    ...
    x = np.zeros(n)  # Начальное приближение
    r = b - A @ x  # Невязка
    w = C_inv @ r  # Направление
    v = C_inv @ w
    solution_history = np.zeros((n_iters, n)) if n_iters is not None else None

    for k in range(n_iters):
        t = np.dot(w, w) / np.dot(v, A @ v)
        x = x + t * v
        r_new = r - t * A @ v
        w_new = C_inv @ r_new
        s = np.dot(w_new, w_new) / np.dot(w, w)
        v = C_inv @ w_new + s * v
        w = w_new
        r = r_new
...
if __name__ == "__main__":
    ...
    C_inv = np.diag(1/ np.sqrt(np.diag(A)))
