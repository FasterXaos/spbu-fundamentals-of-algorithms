...
def power_method(A: NDArrayFloat, n_iters: int) -> tuple[NDArrayFloat, NDArrayFloat]:
    ...
    eigenvalue_history = np.zeros(n_iters)
    eigenvector_history = np.zeros((n_iters, A.shape[0]))

    for i in range(n_iters):
        x = A @ x
        x /= np.linalg.norm(x)

        eigenvalue_history[i] = np.dot(x, A @ x)
        eigenvector_history[i] = x

    return eigenvalue_history, eigenvector_history
